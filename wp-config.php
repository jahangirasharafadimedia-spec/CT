...
if ( file_exists( dirname( __FILE__ ) . '/wp-local-config.php' ) ) {
	require( dirname( __FILE__ ) . '/wp-local-config.php' );
} else {
	die( 'ERROR: wp-local-config.php is not present!' );
}
...
/** Enable REST API (ensure it's not disabled) */
if ( ! defined( 'REST_API_DISABLED' ) ) {
	define( 'REST_API_DISABLED', false );
}
